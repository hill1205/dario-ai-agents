...
export default function IAGREXPage({ fontSize=14, onBack }) {
  ...
  const Cell = ({style={},children}) => (
        <div style={{padding:"10px 12px",fontSize:fs-2,color:"#94A3B8",display:"flex",alignItems:"center",...style}}>{children}</div>div>
      );
        ...
        <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden",background:"#09090F"}}>
                ...
                <div style={{padding:"14px 20px",borderBottom:"1px solid #1A1A2E",flexShrink:0}}>
                          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                                        {onBack && <button onClick={onBack} style={{padding:"5px 10px",borderRadius:7,border:"1px solid #1A1A2E",background:"transparent",color:"#64748B",cursor:"pointer",fontSize:12}}>← Home</button>button>}
                                                    <div style={{fontWeight:700,fontSize:15,color:"#F8FAFC"}}>📊 Finanze IAGREX</div>div>
                                        {saveStatus==="saving"  && <span style={{fontSize:11,color:"#F59E0B"}}>☁️ Salvataggio...</span>span>}
                                        {saveStatus==="saved"   && <span style={{fontSize:11,color:"#10B981"}}>✅ Salvato</span>span>}
                                        {saveStatus==="error"   && <span style={{fontSize:11,color:"#EF4444"}}>❌ Errore</span>span>}
                                        {saveStatus==="blocked" && <span style={{fontSize:11,color:"#EF4444"}}>🚫 Salvataggio bloccato: dati non caricati</span>span>}
                                      </div>div>
                                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                                                    <button onClick={prevMonth} style={{width:28,height:28,borderRadius:6,border:"1px solid #1A1A2E",background:"transparent",color:"#64748B",cursor:"pointer",fontSize:14}}>‹</button>button>
                                                    <span style={{fontSize:fs-1,fontWeight:700,color:"#F8FAFC",minWidth:140,textAlign:"center"}}>{getMonthLabel(month)}</span>span>
                                                    <button onClick={nextMonth} style={{width:28,height:28,borderRadius:6,border:"1px solid #1A1A2E",background:"transparent",color:"#64748B",cursor:"pointer",fontSize:14}}>›</button>button>
                                                    <button onClick={loadData} style={{padding:"4px 8px",borderRadius:6,border:"1px solid #1A1A2E",background:"transparent",color:"#475569",cursor:"pointer",fontSize:11}}>{loading?"⏳":"↻"}</button>button>
                                      </div>div>
                          </div>div>
                  {loadError && (
                  ...
                  {/* YTD Progress */}
                          <div style={{marginTop:12,background:"#0F0F1A",border:"1px solid #10B98130",borderRadius:10,padding:"12px 14px"}}>
                                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                                                    <div style={{fontSize:fs-3,color:"#64748B"}}>Progress {year} verso 1.000.000€</div>div>
                                                    <div style={{fontSize:fs-2,fontWeight:700,color:"#10B981"}}>{fmt(ytdRevenue)}€ · {ytdPct}%</div>div>
                                      </div>div>
                                      ...
                                      <div style={{fontSize:fs-4,color:"#334155",marginTop:4}}>Mancano {fmt(OBIETTIVO_ANNUO-ytdRevenue)}€ all'obiettivo annuo</div>div>
                          </div>div>
          ...
          {label:"MRR stimato",  val:totEntrate,color:"#3B82F6",prefix:""},
                                  ...
                                  <div key={c.label} style={{background:"#0F0F1A",border:"1px solid #1A1A2E",borderRadius:8,padding:"8px 12px"}}>
                                                  <div style={{fontSize:fs-4,color:"#475569",marginBottom:3}}>{c.label}</div>div>
                                                  <div style={{fontSize:fs+1,fontWeight:800,color:c.color}}>{c.prefix}{fmt(c.val)}€</div>div>
                                  </div>div>
                                ))}
                          </div>div>
                </div>div>
          ...
          {/* Tabs */}
                <div style={{display:"flex",borderBottom:"1px solid #1A1A2E",flexShrink:0,background:"#09090F"}}>
                  {[["entrate","💚 Entrate"],["uscite","🔴 Uscite"],["saldi","🏦 Saldi"]].map(([t,label])=>(
                    <button key={t} onClick={()=>setTab(t)} style={{padding:"10px 16px",border:"none",background:"transparent",cursor:"pointer",fontSize:fs-2,fontWeight:tab===t?700:400,color:tab===t?"#F8FAFC":"#475569",borderBottom:tab===t?"2px solid #3B82F6":"2px solid transparent"}}>{label}</button>button>
                  ))}
                </div>div>

          {loading && <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",color:"#334155"}}>⏳ Caricamento...</div>div>}
          ...
          {!loading && (
                  <div style={{flex:1,overflowY:"auto",padding:16}}>
                    ...
                    {tab==="entrate" && (
                                <div>
                                              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                                                              <div style={{fontSize:fs-2,color:"#64748B"}}>Totale: <span style={{color:"#10B981",fontWeight:700}}>+{fmt(totEntrate)}€</span>span></div>div>
                                                              <button onClick={()=>openAdd("entrata")} style={{padding:"6px 14px",borderRadius:7,border:"none",background:"#10B981",color:"#fff",cursor:"pointer",fontSize:12,fontWeight:600}}>+ Aggiungi</button>button>
                                              </div>div>
                                              <div style={{background:"#0F0F1A",border:"1px solid #1A1A2E",borderRadius:10,overflow:"hidden"}}>
                                                {monthData.entrate.length===0
                                                                    ? <div style={{padding:20,textAlign:"center",color:"#334155",fontSize:fs-2}}>Nessuna entrata — aggiungi la prima</div>div>
                                                                : monthData.entrate.map((e,i)=>(
                                                                  <div key={e.id} style={{display:"grid",gridTemplateColumns:"1fr auto auto auto",gap:0,borderTop:i===0?"none":"1px solid #1A1A2E",background:i%2===0?"#0F0F1A":"#0B0B16"}}>
                                                                                        <Cell style={{flexDirection:"column",alignItems:"flex-start",gap:2}}>
                                                                                                                <span style={{color:"#E2E8F0",fontWeight:600}}>{e.descrizione}</span>span>
                                                                                                                <span style={{fontSize:fs-4,color:"#475569"}}>{e.categoria}{e.cliente?` · ${e.cliente}`:""}</span>span>
                                                                                          </Cell>Cell>
                                                                                        <Cell style={{color:"#10B981",fontWeight:700}}>+{fmt(e.importo)}€</Cell>Cell>
                                                                                        <Cell><button onClick={()=>openEdit("entrata",e)} style={{width:24,height:24,borderRadius:5,border:"1px solid #1A1A2E",background:"transparent",color:"#64748B",cursor:"pointer",fontSize:10}}>✏️</button>button></Cell>Cell>
                                                                                        <Cell><button onClick={()=>deleteItem("entrata",e.id)} style={{width:24,height:24,borderRadius:5,border:"1px solid #2A1A1A",background:"transparent",color:"#EF4444",cursor:"pointer",fontSize:12,fontWeight:700}}>×</button>button></Cell>Cell>
                                                                  </div>div>
                                                                ))
                                                }
                                              </div>div>
                    ...
                    {tab==="uscite" && (
                                <div>
                                              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                                                              <div style={{fontSize:fs-2,color:"#64748B"}}>Totale: <span style={{color:"#EF4444",fontWeight:700}}>-{fmt(totUscite)}€</span>span></div>div>
                                                              <button onClick={()=>openAdd("uscita")} style={{padding:"6px 14px",borderRadius:7,border:"none",background:"#EF4444",color:"#fff",cursor:"pointer",fontSize:12,fontWeight:600}}>+ Aggiungi</button>button>
                                              </div>div>
                                              <div style={{background:"#0F0F1A",border:"1px solid #1A1A2E",borderRadius:10,overflow:"hidden"}}>
                                                {monthData.uscite.length===0
                                                                    ? <div style={{padding:20,textAlign:"center",color:"#334155",fontSize:fs-2}}>Nessuna uscita</div>div>
                                                                : monthData.uscite.map((e,i)=>(
                                                                  <div key={e.id} style={{display:"grid",gridTemplateColumns:"1fr auto auto auto",gap:0,borderTop:i===0?"none":"1px solid #1A1A2E",background:i%2===0?"#0F0F1A":"#0B0B16"}}>
                                                                                        <Cell style={{flexDirection:"column",alignItems:"flex-start",gap:2}}>
                                                                                                                <span style={{color:"#E2E8F0",fontWeight:600}}>{e.descrizione}</span>span>
                                                                                                                <span style={{fontSize:fs-4,color:"#475569"}}>{e.categoria}</span>span>
                                                                                          </Cell>Cell>
                                                                                        <Cell style={{color:"#EF4444",fontWeight:700}}>-{fmt(e.importo)}€</Cell>Cell>
                                                                                        <Cell><button onClick={()=>openEdit("uscita",e)} style={{width:24,height:24,borderRadius:5,border:"1px solid #1A1A2E",background:"transparent",color:"#64748B",cursor:"pointer",fontSize:10}}>✏️</button>button></Cell>Cell>
                                                                                        <Cell><button onClick={()=>deleteItem("uscita",e.id)} style={{width:24,height:24,borderRadius:5,border:"1px solid #2A1A1A",background:"transparent",color:"#EF4444",cursor:"pointer",fontSize:12,fontWeight:700}}>×</button>button></Cell>Cell>
                                                                  </div>div>
                                                                ))
                                                }
                                              </div>div>
                    ...
                    {tab==="saldi" && (
                                <div>
                                              <div style={{fontSize:fs-3,fontWeight:700,color:"#3B82F6",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8}}>🏦 Saldi Conti IAGREX</div>div>
                                              <div style={{background:"#0F0F1A",border:"1px solid #1A1A2E",borderRadius:10,overflow:"hidden"}}>
                                                {CONTI_IAGREX.map((c,i)=>(
                                                    <div key={c.id} style={{display:"grid",gridTemplateColumns:"1fr auto",borderTop:i===0?"none":"1px solid #1A1A2E",background:i%2===0?"#0F0F1A":"#0B0B16"}}>
                                                                        <div style={{padding:"10px 12px",fontSize:fs-2,color:"#E2E8F0",display:"flex",alignItems:"center"}}>{c.label}</div>div>
                                                                        <div style={{padding:"6px 12px",display:"flex",alignItems:"center",gap:4}}>
                                                                                              ...
                                                                                              <span style={{fontSize:fs-3,color:"#475569"}}>{c.currency}</span>span>
                                                                        </div>div>
                                                    </div>div>
                                                  ))}
                                                              <div style={{padding:"10px 12px",borderTop:"1px solid #1A1A2E",background:"#09090F",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
                                                                                <div>
                                                                                                    <div style={{fontSize:fs-2,fontWeight:700,color:"#3B82F6"}}>Totale liquidità aziendale</div>div>
                                                                                                    <div style={{fontSize:fs-4,color:"#334155",marginTop:2}}>EUR convertiti a RON (×{EUR_RON_RATE})</div>div>
                                                                                </div>div>
                  ...
                  <div style={{position:"fixed",inset:0,background:"#00000090",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={closeModal}>
                            <div style={{background:"#0F0F1A",border:"1px solid #1A1A2E",borderRadius:16,padding:24,width:"100%",maxWidth:400,maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
                                        ...
                                        <div style={{display:"flex",flexDirection:"column",gap:12}}>
                                                      <div>
                                                                      <div style={{fontSize:11,color:"#64748B",marginBottom:4}}>Descrizione *</div>div>
                                          ...
                                          {modal.tipo==="entrata" && (
                                    <div>
                                                      <div style={{fontSize:11,color:"#64748B",marginBottom:4}}>Cliente</div>div>
                                                      <input type="text" value={form.cliente||""} onChange={e=>setForm(p=>({...p,cliente:e.target.value}))} placeholder="Nome cliente..."
                                                                            style={{width:"100%",padding:"8px 10px",borderRadius:7,border:"1px solid #1A1A2E",background:"#09090F",color:"#E2E8F0",fontSize:13,outline:"none"}}/>
                                    </div>div>
                                                      )}
                                                      <div>
                                                                      <div style={{fontSize:11,color:"#64748B",marginBottom:4}}>Importo € *</div>div>
                                                                      <input type="number" value={form.importo||""} onChange={e=>setForm(p=>({...p,importo:e.target.value}))}
                                                                                          style={{width:"100%",padding:"8px 10px",borderRadius:7,border:"1px solid #1A1A2E",background:"#09090F",color:"#E2E8F0",fontSize:13,outline:"none"}}/>
                                                      </div>div>
                                                      <div>
                                                                      <div style={{fontSize:11,color:"#64748B",marginBottom:6}}>Categoria</div>div>
                                        ...
                                        <button key={c} onClick={()=>setForm(p=>({...p,categoria:c}))}
                                          ...
                                          {modal.tipo==="uscita" && (
                                    <div>
                                                      <div style={{fontSize:11,color:"#64748B",marginBottom:4}}>Pagato da 🏦</div>div>
                                                      <select value={form.conto||""} onChange={e=>setForm(p=>({...p,conto:e.target.value}))}
                                                                            style={{width:"100%",padding:"8px 10px",borderRadius:7,border:"1px solid #1A1A2E",background:"#09090F",color:"#E2E8F0",fontSize:13,outline:"none"}}>
                                                                          <option value="">-- Seleziona conto --</option>option>
                                                        {CONTI_IAGREX.map(c=><option key={c.id} value={c.id}>{c.label}</option>option>)}
                                                      </select>select>
                                    </div>div>
                                                      )}
                                        </div>div>
                                        <div style={{display:"flex",gap:8,marginTop:20}}>
                                                      <button onClick={closeModal} style={{flex:1,padding:10,borderRadius:8,border:"1px solid #1A1A2E",background:"transparent",color:"#475569",cursor:"pointer",fontSize:13}}>Annulla</button>button>
                                                      <button onClick={saveItem} style={{flex:2,padding:10,borderRadius:8,border:"none",background:modal.tipo==="entrata"?"#10B981":"#EF4444",color:"#fff",cursor:"pointer",fontSize:13,fontWeight:700}}>Salva</button>button>
                                        </div>div>
                            </div>div>
                  </div>div>
              )}
